...
class DeConvModule(torch.nn.Module):
    def __init__(
        self,
        conv_kernel,
        in_channel,
...
class BaselineModel(lightning.LightningModule):
    ...
    def __init__(
        ...
        self.classifier = torch.nn.Sequential(
            torch.nn.Linear(self.latent_size, 450),
            torch.nn.BatchNorm1d(450, affine=False),
            torch.nn.PReLU(),
             torch.nn.Linear(450, 450),
            torch.nn.BatchNorm1d(450, affine=False),
            torch.nn.PReLU(),
            torch.nn.Linear(450, 128),
            torch.nn.BatchNorm1d(128, affine=False),
            torch.nn.PReLU(),
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.use_decoder:
            ...
            model_loss_tot = recon_loss  +  label_loss
